...
WITH SR AS (
  ...
  ,(CASE WHEN TRUNC(sr.TRANS_DTM) IN ('24-MAY-18','06-JUL-18') AND item.IS_GV_ESP = 1 THEN 'HHITEM' 
      ...
      WHEN  TRUNC(sr.TRANS_DTM) = '07-JUN-18' AND item.IS_GV_LM = 0 THEN 'NON-HHITEM' 
            WHEN TRUNC(sr.TRANS_DTM) = '15-JUN-18' AND item.IS_G_FRAPP = 1 THEN 'HHITEM' 
      WHEN  TRUNC(sr.TRANS_DTM) = '15-JUN-18' AND item.IS_G_FRAPP = 0 THEN 'NON-HHITEM' 
            WHEN TRUNC(sr.TRANS_DTM) IN ('21-JUN-18','19-JUL-18') AND item.IS_GV_FRAPP = 1 THEN 'HHITEM' 
      WHEN  TRUNC(sr.TRANS_DTM) IN ('21-JUN-18','19-JUL-18') AND item.IS_GV_FRAPP = 0 THEN 'NON-HHITEM' 
...
JOIN APPCA.F_POS_LINE_ITEM pi
  ON TO_CHAR(sr.TRANS_DTM, 'YYYYMMDD') || TRIM(TO_CHAR(sr.STORE_NUM, '000000'))
    ...
    AND pi.CNTRY_CD = 'US'
    AND pi.BUS_DT IN ('24-MAY-18', '07-JUN-18', '15-JUN-18', '21-JUN-18', '29-JUN-18', '06-JUL-18', '12-JUL-18', '19-JUL-18')
...
WHERE sr.QSTN_ID IN ('Q2_2')
  ...
  AND ca.CAL_DT IN ('24-MAY-18', '07-JUN-18', '15-JUN-18', '21-JUN-18', '29-JUN-18', '06-JUL-18', '12-JUL-18', '19-JUL-18')
...
SR2 AS (
  ...
  ,COUNT(*) AS TOTAL_RESP
  ,ROUND(SUM(TOTAL_TB) / COUNT(*),4) AS TB_SCORE
  ,ROUND(STDDEV(TOTAL_TB),4) AS TB_SCORE_SD
  ,ROUND(AVG(SR2.VISITS_PRE30D),2) AS VISITS_PRE30D_AVG
  ,ROUND(STDDEV(SR2.VISITS_PRE30D),2) AS VISITS_PRE30D_SD
  ,ROUND(AVG(SR2.VISITS_POST30D),2) AS VISITS_POST30D_AVG
  ,ROUND(STDDEV(SR2.VISITS_POST30D),2) AS VISITS_POST30D_SD
  ,SR2.HH_ITEM
  ,SR2.DAY_PART
